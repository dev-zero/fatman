...
class TestResult2Collection(Base):
    ...
    autogenerated_for_id = Column(UUID(as_uuid=True), ForeignKey('calculation_collection.id'),
                                  nullable=True, unique=True)  # ensure no 2 autogenerated collections for
                                                               # the same calculation collection
    autogenerated_for = relationship("CalculationCollection")
    ...
    @hybrid_property
    def readonly(self):
        return self.autogenerated_for_id is not None
